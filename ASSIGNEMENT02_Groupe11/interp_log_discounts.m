...
%initial_date              date at which discount is computed 
 ...
 if       settlement_date == expiry_prev_future  % if they coincides
...
else if   settlement_date > expiry_prev_future %extrapolation
          ...
          r = interp1([settlement_prev_future , expiry_prev_future],[settlement_discount, expiry_discount],settlement_date,'linear','extrap');
          
          
else     settlement_future < expiry_prev_future % interpolation
    
          r1 = -log(settlement_discount)/yearfrac(initial_date, settlement_prev_future,3);
          r2 = -log(expiry_discount)/yearfrac(initial_date, expiry_prev_future,3);
          r = interp1([settlement_prev_future , expiry_prev_future],[settlement_discount, expiry_discount],settlement_date,'linear');
